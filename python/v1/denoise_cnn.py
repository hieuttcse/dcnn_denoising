...
class SDCNN(object): # Simple Denosing CNN
    ...
    def train(self,flags):
        ...
        optimizerDenoising = tf.train.AdamOptimizer(flags.learning_rate).minimize(self.MSE,global_step=globalStep1)
        ...
        noisypTrain = noisyPatches[noTrain:,:,:]
        ...
        for epoch in range(100000):
            ...
            for bIdx in range(0,noBatches):
                ...
                in1 = np.zeros([self.batchSize,self.patchSize,self.patchSize,1])
                gt1 = np.zeros([self.batchSize,self.patchSize,self.patchSize,1])

                [val_MSE] = self.sess.run([optimizerDenoising,self.MSE],
    ...
    def denoising_net(self,input_):
       with tf.variable_scope(tf.get_variable_scope()):
           ...
           h1 = self.conv2d(input,32, k_h=5,k_w=5,d_h=1,d_w=1,padding='SAME',name='one')
           h1 = self.tf.nn.relu(h1)
